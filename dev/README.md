...
1. **Start task**: Update status in `backlog.edn` and `current.edn`
1. **During work**: Update `log.md` with notes and decisions
1. **Complete task**:
   - Mark as `:done` in both EDN files
   - Add commit SHAs to `:commits`
   - Create changelog fragment in `changelog/unreleased/`
1. **End session**: Update `current.edn` with completed work and next steps
